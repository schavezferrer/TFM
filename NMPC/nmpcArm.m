function [t, x, u, fval] = nmpcArm(runningcosts, terminalcosts, ...
...
%   lastU:          Last control signal applied //SEBAS
%
% Optional arguments:
...
% (C) Lars Gruene, Juergen Pannek 2011
    ...
    while(mpciter < mpciterations)
        ...
        [u_new, V_current, exitflag, output] = solveOptimalControlProblem ...
            ...
            terminalconstraints, linearconstraints, system, ...
            N, t0, x0, u0,lastU, T,baseReaction, ...
...
function [tapplied, xapplied] = applyControl(system, T, t0, x0, u,baseReaction, ...
                                atol_ode_real, rtol_ode_real, type)
    xapplied = dynamic(system, T, t0, x0, u(:,1),baseReaction, ...
                       atol_ode_real, rtol_ode_real, type);
    tapplied = t0+T;
end
...
function [u, V, exitflag, output] = solveOptimalControlProblem ...
    ...
    linearconstraints, system, N, t0, x0, u0,lastU, T,baseReaction, ...
    atol_ode_sim, rtol_ode_sim, tol_opt, options, type)
    x = zeros(N+1, length(x0));
    x = computeOpenloopSolution(system, N, T, t0, x0, u0,baseReaction, ...
    ...
    for k=1:N
        [Anew, bnew, Aeqnew, beqnew, lbnew, ubnew] = ...
               linearconstraints(t0+k*T,x(k,:),u0(:,k),lastU);
        A = blkdiag(A,Anew);
        b = [b, bnew];
    ...
    [u, V, exitflag, output] = fmincon(@(u) costfunction(runningcosts, ...
        ...
        u,lastU, baseReaction,atol_ode_sim, rtol_ode_sim, type), ...
        u0, A, b, Aeq, beq, lb, ...
        ub, @(u) nonlinearconstraints(constraints, terminalconstraints, ...
        system, N, T, t0, x0, u,baseReaction, ...
        atol_ode_sim, rtol_ode_sim, type), options);
    
end

function cost = costfunction(runningcosts, terminalcosts, system, ...
                    N, T, t0, x0, u,lastU, baseReaction,...
                    atol_ode_sim, rtol_ode_sim, type)
    cost = 0;
    x = zeros(N+1, length(x0));
    x = computeOpenloopSolution(system, N, T, t0, x0, u,baseReaction, ...
                                atol_ode_sim, rtol_ode_sim, type);
    lastControl =  lastU;                       
    for k=1:N
        cost = cost+runningcosts(t0+k*T, x(k,:), u(:,k),lastControl);
        lastControl = u(:,k);
        %cost = cost+runningcosts(t0+k*T, x(k,:), u(:,k),u(:,k-1));
...
function [c,ceq] = nonlinearconstraints(constraints, ...
    ...
    N, T, t0, x0, u,baseReaction, atol_ode_sim, rtol_ode_sim, type)
    x = zeros(N+1, length(x0));
    x = computeOpenloopSolution(system, N, T, t0, x0, u,baseReaction, ...
                                atol_ode_sim, rtol_ode_sim, type);
    c = [];
    ceq = [];
    for k=1:N
        [cnew, ceqnew] = constraints(t0+k*T,x(k,:),u(:,k),N);
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
...
%         u(4*k-3:4*k,1)
        x(k+1,:) = dynamic(system, T, t0+(k-1)*T, x(k,:), u(:,k),baseReaction, ...
                             atol_ode_sim, rtol_ode_sim, type);
    end
end
...
function printSolution(system, printHeader, printClosedloopData, ...
    ...
    switch exitflag
        ...
        case 1
        if ( iprint == 1 )
            fprintf('Almost right\n');
        elseif ( iprint >= 2 && iprint < 10 )
            fprintf(['Could be a problem with 1st-order optimality',... 
                    'or constraints\n']);
        elseif ( iprint >= 10 )
        ...
        case 3
        if ( iprint == 1 )
            fprintf('Some troubles in objective function\n');
        elseif ( iprint >= 2 && iprint < 10 )
            fprintf(' Warning TJ\n');
        elseif ( iprint >= 10 )
        ...
        case 4
        if ( iprint == 1 )
            fprintf('Could be a search direction problem\n');
        elseif ( iprint >= 2 && iprint < 10 )
            fprintf(' Warning S\n');
        elseif ( iprint >= 10 )
        ...
        case 5
        if ( iprint == 1 )
            fprintf('Could be a magnitude of directional derivative problem\n');
        elseif ( iprint >= 2 && iprint < 10 )
            fprintf(' Warning D\n');
        elseif ( iprint >= 10 )
