...
function [allData, t, x, u] = loopClosed
    ...
    k = 1; % Velocidad angular de los motores inicial
    w1 = k*ones(1,N);
    w2 = 0*k*ones(1,N);
    w3 = 0*k*ones(1,N);
    w4 = 0*k*ones(1,N);
    ...
    if(currSample >= 3)
        ...
        uCompo = [u(1,1) lastU(1) u(2,1) lastU(2) u(3,1) lastU(3) u(4,1) lastU(4)];
    
        velCompo = [  vel(currSample-1,1) vel(currSample-2,1)
                    vel(currSample-1,2) vel(currSample-2,2)
                    vel(currSample-1,3) vel(currSample-2,3)
                    vel(currSample-1,4) vel(currSample-2,4)
                    vel(currSample-1,5) vel(currSample-2,5)
                    vel(currSample-1,6) vel(currSample-2,6)   ];
            
        vel(currSample,:) = sim(SiSmooth,[velCompo uCompo]');
...
function [y] = system(t, x, u, T, lastU)
  

      uant = [lastU(1,1) lastU(5,1)
              lastU(2,1) lastU(6,1)
              lastU(3,1) lastU(7,1)
              lastU(4,1) lastU(8,1)];
          
      y =  sim(SiSmooth,u,'init',[yant uant]');
